...
export function ChatSidebar({ isOpen, onClose, onNewChat }: ChatSidebarProps): React.JSX.Element {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-y-0 inset-s-0 z-50 flex w-72 flex-col border-e border-border bg-background/85 backdrop-blur-xl transition-transform duration-300 md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : isRtl ? 'translate-x-full' : '-translate-x-full'
          }`}
      >
        ...
        <div className="flex h-16 items-center justify-between px-6 border-b border-border/40 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold tracking-tight text-foreground">{t('common.appTitle')}</span>
          </div>
          <Button
            variant="ghost"
            size="icon"
                  ...
                  >
                    <div className="flex items-center gap-2 overflow-hidden">
                      ...
                      <span dir="auto" className="truncate">
                        {['محادثة جديدة', 'New Chat', 'new_chat', 'New chat'].includes(s.title)
                          ? t('common.newChat')
                          : s.title}
                      </span>
                    </div>
